...
public class MetaWeightTest {
    
    @Autowired
    MetaWeightRepository repository;
    
    @Test
    public void create() throws Exception {
	final String name = "createTestWeightName";
	...
	Long id = null;
	{
	    ...
	    MetaWeight created = repository.save(weight);
	    id = created.getId();
	}
	
	MetaWeight weight = repository.findOne(id);
	assertEquals(name, weight.getName());
	assertEquals(value, weight.getValue());
	assertNotNull(weight.getUpdatedTime());	
    }
}
